"""Temperature Guided Diffusion Experiment Script.

This script is designed to run experiments using temperature-guided diffusion for offline reinforcement
learning tasks. It supports training, testing, and optimization of sampling parameters using Optuna. The script is
...
def test(cfg: DictConfig):
    ...
    trainer, dataset, renderer = instantiate_config(cfg)
    episodes = trainer.rollout_episodes(100, return_plans="maze2d" in cfg.env)
...
def optim(cfg: DictConfig, use_optuna_distributed: bool = True):
    ...
    # Set the default parameters as a starting point.
    study.enqueue_trial(
        {
            "guide_scale": cfg.diffusion_model.guide_scale,
            "guide_scale_alpha": cfg.diffusion_model.guide_scale_alpha,
        }
    )
    study.optimize(
...
def _optimize_sampling_params(
    ...
    # Search space.
    guide_scale = trial.suggest_float("guide_scale", 0.0, 40.0)
    guide_scale_alpha = trial.suggest_float("guide_scale_alpha", 0.0, 10.0)
    cfg_cp.diffusion_model.guide_scale = guide_scale
    cfg_cp.diffusion_model.guide_scale_alpha = guide_scale_alpha
    # Run the inference.
...
if __name__ == "__main__":
    ...
    # Load configuration.
    config = load_config(args.env, "config/experiments/temperature_guided_diffusion_veteran", overrides)
