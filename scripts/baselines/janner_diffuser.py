"""Reimplementation of Janner et al. - Planning with Diffusion for Flexible Behavior Synthesis."""
...
def test(cfg: DictConfig):
    ...
    logger.info(f"Return mean ({cfg.experiment_name}): {np.mean(returns)}, std: {np.std(returns)}")
...
def optim(cfg: DictConfig, use_optuna_distributed: bool = True):
    ...
    assert "maze2d" not in cfg.env, "Maze2D environments are not supported for optimization."
    ...
    # Set the Janner parameters as a starting point.
    study.enqueue_trial(
        {
            "guide_scale": cfg.diffusion_model.guide_scales.trajectory_return,
            "stop_grad": cfg.diffusion_model.sigma_stop_grad,
        }
    )
    study.optimize(lambda trial: _optimize_sampling_params(trial, cfg), n_trials=100, n_jobs=4)
...
def _optimize_sampling_params(
    ...
    # Search space.
    guide_scale = trial.suggest_float("guide_scale", 0.0001, 0.5, log=True)
    stop_grad = trial.suggest_float("stop_grad", 0.0, 0.5)
    cfg_cp.diffusion_model.guide_scales.trajectory_return = guide_scale
    cfg_cp.diffusion_model.sigma_stop_grad = stop_grad
    # Run the inference.
...
if __name__ == "__main__":
    ...
    # Load configuration.
    config = load_config(args.env, "config/baselines/janner_diffuser", overrides)
